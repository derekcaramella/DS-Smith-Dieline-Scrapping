...
for dieline in os.listdir('DS Smith Dielines'):  # Loop through the directory, which analyzes each dieline
    ...
    for instance in list_prompts:  # Within one file, loop through ever instance
        ...
        if 'FinishedPieceWeight:' in instance:  # If Finished Piece Weight: exists, capture process value
            finished_piece_weight = instance.partition('FinishedPieceWeight:')[2]
        if 'Over-allBlank:' in instance:  # If Over-all Blank: exists, capture process value
            over_all_blank = instance.partition('Over-allBlank:')[2]  # Bifurcate string & return process value
...
export_df = pd.DataFrame.from_records(df_list, columns=['File', 'Design', 'Description', 'Style', 'Board',
                                                        'Inner Dimension', 'Finished Piece Weight',
                                                        'Over-all Blank', 'Combination'])
# Export dataframe
export_df.to_excel('DS Smith Specifications.xlsx', index=False)
